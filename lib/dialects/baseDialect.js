...
dialect.buildSelectStatement = function (Type, options) {
    ...
    sql += fieldRefs.join(", ") + "FROM " + Type.table + ' ' + rootTblAlias + " " + joins.joinList.join(' ');
...
function _buildJoins(Type, rootAlias, joinOptions) {
    ...
    for(opt in joinOptions) {
        ...
        joinList.push('INNER JOIN ' + ChildType.table + ' ' + alias + ' ON ' + alias + '.' + ChildType.identifier + ' = ' + rootAlias + "." + joinOptions[opt].on);
