...
dorm.config = function config (fileName) {
...
dorm.update = function (entity, handler) {
    if (!entity) {
        throw new Error("ValueError: cannot save undefined or null entity");
    }
    if (entity.id) {
        this.save(entity, handler);
    } else {
        throw new Error('ValueError: Cannot update value with undefined or null primary key field');
    }
}
